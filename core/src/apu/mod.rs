...
use crate::{
    bus::MemoryAccess,
    cpu::CPU_CLOCK_SPEED,
    scheduler::event::{ApuEvent, EventType, FutureEvent},
};
use std::{
...
impl Apu {
    ...
    pub fn handle_event(&mut self, apu_event: ApuEvent) -> Option<FutureEvent> {
    ...
    fn handle_sample(&mut self) -> (ApuEvent, usize) {
        ...
        (ApuEvent::Sample, CYCLES_PER_SAMPLE as usize)
    }
